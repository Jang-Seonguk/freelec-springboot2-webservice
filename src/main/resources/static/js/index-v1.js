...
async function runCompatibilityCheck() {
    ...
    try {
        const compatibilityResult = await apiCall('/compatibility/check', {
            method: 'POST',
            body: JSON.stringify({
        ...
        if (compatibilityResult.compatible) {
            alert('✅ All components are compatible!');
        } else {
...
async function searchAllParts() {
    ...
    try {
        const searchResults = await apiCall(`/parts/search/all?keyword=${encodeURIComponent(searchTerm)}`);
        alert(`Found ${searchResults.length} parts matching "${searchTerm}"`);
    } catch (error) {
